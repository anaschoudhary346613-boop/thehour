...
export default function AdminPage() {
  ...
  useEffect(() => {
    async function checkAdmin() {
      ...
      // Strict Admin Check (Replace with your actual Admin UUID)
      ...
      if (!session || session.user.id !== ADMIN_UUID) {
        // For demonstration, we'll allow access if no env is set, but warn
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#050505] flex items-center justify-center p-6">
       <span className="text-[#C8A97E] text-[10px] uppercase tracking-widest animate-pulse font-bold">Initializing Admin Sectors...</span>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans flex flex-col md:flex-row">
      ...
      <aside className="w-full md:w-80 bg-[#0A0A0A] border-r border-white/5 p-12 flex flex-col gap-12">
         <div className="space-y-4">
            ...
            <h1 className="text-3xl font-serif uppercase tracking-tight leading-none text-white">Ops Center</h1>
         </div>
         ...
         {activeTab === 'orders' ? (
           <div className="space-y-12">
              <div className="flex justify-between items-end">
                 <h2 className="text-4xl font-serif uppercase tracking-tight">Incoming Acquisitions</h2>
                 ...
                 <table className="w-full text-left border-collapse">
                    <thead>
                       <tr className="bg-white/[0.03]">
                          <th className="p-6 text-[9px] uppercase tracking-widest text-[#C8A97E] font-black">Transfer ID</th>
                          <th className="p-6 text-[9px] uppercase tracking-widest text-[#C8A97E] font-black">Collector</th>
                         ...
                         <tr key={order.id} className="hover:bg-white/[0.02] transition-colors group">
                            ...
                            <td className="p-6">
                               <div className="flex flex-col">
                                  <span className="text-xs font-bold text-white uppercase">{order.user_id ? 'Authenticated Collector' : 'Direct Inquiry'}</span>
           ...
           <div className="space-y-12">
              <div className="flex justify-between items-end">
                 <h2 className="text-4xl font-serif uppercase tracking-tight">Vault Inventory</h2>
                 ...
                 {watches.map((watch) => (
                   <div key={watch.id} className="bg-white/[0.02] border border-white/5 rounded-3xl p-8 flex gap-8 items-center group relative overflow-hidden">
                      <div className="w-24 h-24 bg-black/40 rounded-2xl p-4 border border-white/5 flex-shrink-0 relative z-10">
                         <img src={watch.hero_image_url} alt="Artifact" className="w-full h-full object-contain" />
                      </div>
                      <div className="flex-1 space-y-2 relative z-10">
                         <div className="flex justify-between items-start">
                            ...
                            <span className={`text-[8px] font-black uppercase tracking-[0.2em] px-3 py-1 rounded-full border ${watch.is_featured ? 'border-[#C8A97E] text-[#C8A97E] bg-[#C8A97E]/5' : 'border-white/10 text-white/30'}`}>
                               {watch.is_featured ? 'Featured' : 'Standard'}
                            </span>
                         </div>
                         <div className="flex items-center gap-6">
                            <span className="text-[#C8A97E] font-serif text-lg">{formatPrice(watch.price)}</span>
                            <span className="text-[9px] text-white/20 uppercase tracking-widest font-bold">QTY: {watch.stock || 0}</span>
                         </div>
                         <div className="flex gap-4 pt-4">
                            <button className="flex items-center gap-2 text-[9px] uppercase tracking-widest text-white/40 hover:text-white transition-colors font-bold"><Edit3 size={14} /> Modify</button>
                            <button className="flex items-center gap-2 text-[9px] uppercase tracking-widest text-red-500/40 hover:text-red-500 transition-colors font-bold"><Trash2 size={14} /> Remove</button>
                         </div>
                      </div>
                   </div>
                 ))}
